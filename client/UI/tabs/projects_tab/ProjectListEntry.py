...
class ProjectListEntry(QDoubleLabel):
    ...
    def _sync_all_dlg_func(self, dc):
        ...
        for f in files_send:
            real_files_send.append(File(JSON=f))
        for f in files_get:
            real_files_get.append(File(JSON=f))
        ...
        if sure[0] == True:
            if len(files_send) == 0 and len(files_get) == 0 and len(files_to_delete_from_server)== 0 and len(files_to_delete_from_client) == 0:
