...
class File:
    def __init__(self, path=None, date_modified=None, size=None, host_pc=SERVER, f_type=FILE, JSON=None):
        ...
        self.f_type = f_type
        if self.path != None and self.host_pc == SERVER and self.date_modified == None and self.f_type == FILE:
            self.date_modified = int(os.path.getmtime(self.path))
        if self.path != None and self.host_pc == SERVER and self.size == None and self.f_type == FILE:
